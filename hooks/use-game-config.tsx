...
export function GameConfigProvider({ children }: { children: ReactNode }) {
	...
	const updateGameConfig = useCallback((patch: GameConfigPatchType) => {
	...
	const setPlayers = useCallback((players: PlayerType[]) => {
		setConfig((currentConfig) => ({
			...currentConfig,
	...
	const value = useMemo(
		...
		[config, loading, updateGameConfig, setPlayers, resetGameConfig],
	);
